...
import { MappedPopularProductType } from "../populates/type"
import { rawImage, rawResult } from "../populates/type/rawTypePopulates"


const getLocalizedName = (product: rawResult, lang: string): string|null => product?.additional_data?.[lang]!="" ?product?.additional_data?.[lang] : null;

const mapping = (len:number,rawData: rawResult[], city: string): {results:MappedPopularProductType[], len: number} => {
    const Populates: MappedProductType[] = rawData.map((product: rawResult) => ({
        ...
        oldPrice: product.stocks[city]?.price_before_discount ?? null,
