...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if(orderBy !== "none_sort"){
    url = `${UrlApiWithDomainV2.getPopulates}?ordering=${orderBy}&offset=${offset}&limit=${POPULATES_PRODUCTS.POPULATES_PER_PAGE}&city=${cityRu}`;
  }else{
    url = `${UrlApiWithDomainV2.getPopulates}?offset=${offset}&limit=${POPULATES_PRODUCTS.POPULATES_PER_PAGE}&city=${cityRu}`
  }
