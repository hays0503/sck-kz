import {
  rawBasketItemType,
  rawBasketType,
} from "api-mapping/basket/_type/rawBasketType";
import type {
...
const mapping = (data: rawBasketType): MappedBasketType => {
  ...
  const mappedData: MappedBasketItemType[] = data.basket_items.map(
    (item: rawBasketItemType) => {
      // Цена
      const price = item.prod.price ? Object.values(item.prod.price).sort((a, b) => b - a)[0] : 0;
      const oldPrice_p = item.prod.old_price_p ? Object.values(item.prod.old_price_p).sort((a, b) => a - b)[0] : null;
      const oldPrice_c = item.prod.old_price_c ? Object.values(item.prod.old_price_c).sort((a, b) => a - b)[0] : null;
      const oldPrice = oldPrice_p || oldPrice_c; 
      const discount = item.prod.discount_amount_p || item.prod.discount_amount_c;
      return {
        count: item.count,
        prod: {
          id: item.prod_id,
