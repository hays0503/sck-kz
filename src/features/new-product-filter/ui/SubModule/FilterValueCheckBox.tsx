...
const FilterValueCheckBox: React.FC<FilterValueCheckBoxProps> = ({
  keyString,
  disabled,
  productIds,
  filterActive,
  ...
  const onChange = (e: CheckboxChangeEvent) => {
    ...
    if (e.target.checked) {
      debounced((prev: number[]) =>
        Array.from(new Set([...prev, ...productIds])),
      );
      setKeyValue((prev: string[]) => [...prev, keyString]);
    } else {
      debounced((prev: number[]) =>
        prev.filter((item: number) => !productIds.includes(item)),
      );
      setKeyValue((prev: string[]) =>
        prev.filter((item: string) => item !== keyString),
      );
    }
  };
  useEffect(() => {
