...
import { Button, Col, Input, Modal, Row, Space, Tour, TourProps, Typography, Popconfirm, Flex } from "antd";
import { useLocale, useTranslations } from "next-intl";
import { ChangeEvent, CSSProperties, useLayoutEffect, useRef, useState } from "react";
...
const SelectCity = () => {
    ...
    const [citiesSearch, setCitiesSearch] = useState<MappedCityType[]| undefined>(cities?.results);
    ...
    if (!cities && isLoading) {
        return <Flex justify="center" align="center" style={style}>{tStatus('zagruzka')}</Flex>;
    }
    if (error) {
        return <Flex justify="center" align="center" style={style}>{tStatus("oshibka")}{error.message}</Flex>;
    }

    if (!cities) {
        return <Flex justify="center" align="center" style={style}>{tStatus("net-dannykh")}</Flex>;
    }
    ...
    const onSearch = (e: ChangeEvent<HTMLInputElement>) => {
        const filteredCities: MappedCityType[] = searchCity(e.target.value, cities.results, locale);
        ...
        <Flex style={style}>
            ...
            <Modal
                ...
                <Space direction="vertical" size={[16, 16]}>
                    ...
                    <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
                        <Col span={24}>
                            <ComponentSelectCityList cities={citiesSearch??cities.results} setCityLocale={setSelectCityLocal} />
                        </Col>
                    </Row>
                </Space>
            </Modal>
