...
const Level2: React.FC<ILevel2Props> = (props) => {
  ...
  const ReturnRatingCount: React.FC = () => {
    ...
    return (
      <meta itemProp='ratingCount' content={(reviews_count ?? 0).toString()} />
    );
  };
    ...
    return (
      ...
      >
        ...
        <meta
          ...
          content={(average_rating ?? 1).toString()}
        />
        <ReturnRatingCount />
      </div>
    );
  ...
  const RatingAndReviews: React.FC = () => {
    if (!average_rating) return <RatingNotFound />;
    if (average_rating < 0) return <RatingNotFound />;
    return (
      <>
    ...
    >
      <Flex vertical>
        <Flex
          vertical
          itemProp='name'
          style={{ width: '100%', height: '3em', overflow: 'hidden' }}
        >
          <Text>{Brand}</Text>
          <Text style={{ color: '#8c8c8c' }}>{NameProduct ?? ''}</Text>
        </Flex>
      ...
      <Flex justify='flex-start' align='start' gap={5} style={{ height: 20 }}>
        <RatingAndReviews />
      </Flex>
    </Flex>
  );
