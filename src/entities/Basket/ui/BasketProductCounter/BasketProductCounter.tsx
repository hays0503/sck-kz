...
const BasketProductCounter: React.FC<BasketProductCounterProps> = ({children}) => {
    
    const uuid:string | null = useReadLocalStorage<string>("uuid_id") // Может быть null
    const {data:countProductInBasket, isLoading, error} = useGetBasketCountSWR(uuid)
    ...
    return <Badge count={countProductInBasket?.results.count ?? 0} size="small" >
        {children}
    </Badge>
} 
